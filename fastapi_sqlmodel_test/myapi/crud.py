...
# def read_notes(note: Note, session:Session) -> Note:
#     notes_db = select(Note)
#     return notes_db

def insert_note(note: Note, session: Session):
    _note = Note(id=note.id,title=note.title,detail=note.detail,created_at=note.created_at,updated_at=note.updated_at,is_archived=note.is_archived,is_public=note.is_public,user_id=note.user_id)
    session.add(_note)
    session.commit()
    session.refresh(_note)
    return _note
